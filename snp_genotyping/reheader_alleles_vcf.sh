...
sed '1d' $uncompressed_alleles_file | awk '{print $1}' | sort | uniq -c | sort -rnk1 | awk '{print "##contig=<ID="$2",length="$1">"}' | grep -e 2R -e 2L -e 3R -e 3L -e X -e Y_unplaced -e UNKN >> header.hr
